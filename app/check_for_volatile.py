...
def check(client):
    ...
    for symb in info:
        if (
            symb["symbol"] in symbolVolumes
            and symb["symbol"].endswith("USDT")
            and symb["symbol"] not in badSymbols
        ):
    ...
    while True:
        ...
        for ticker in info:
            priceChange = (
                abs((pastTickers[ticker["symbol"]] - float(ticker["price"])))
                / pastTickers[ticker["symbol"]]
            )
            if priceChange > (Config.MinRequiredChangeForStartingSettingPair):
                if (
                    ticker["symbol"] in chosenSymbols
                    and ticker["symbol"] not in badSymbols
                    and ticker["symbol"] not in Config.UnpleasantPairs
                ):
                    print(
                        "\n\n---------------------------------------------\n\nThe big one here: {}".format(
                            ticker["symbol"]
                        ),
                        end="",
                    )
